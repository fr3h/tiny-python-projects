...
def test_no_oxford_comma():
     ...
     arg = 'bananas apples dates cherries'
     out = getoutput(f'{prg} {arg} -s --comma')
     expected = ('You are bringing apples, bananas, cherries and dates.')
     assert out.strip() == expected


# --------------------------------------------------
def test_separation_character():
    """Usea a character as a separator for the list items"""

    arg = 'ban,anas app,les dates cherries'
    out = getoutput(f'{prg} {arg} -s --separator ;')
    expected = ('You are bringing app,les; ban,anas; cherries; and dates.')
    assert out.strip() == expected